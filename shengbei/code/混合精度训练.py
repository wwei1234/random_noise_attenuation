...
model_dir = "U_Net3+_N2N_712"
log_dir = r"D:\桌面\项目\shengbei\训练日志(UNet3+)(7_12)"
patch_size = (128, 128)
stride = (128, 128)
batch_size = 32
learning_rate = 1e-4
num_epochs = 200
seed = 30
input_data_path = r"D:\桌面\项目\shengbei\data\shengbei_pstm_cb.npy"
data = np.load(input_data_path)
dataset = N2NDatasetFromCrossline(data, patch_size=patch_size, stride=stride)
...
for epoch in range(num_epochs):
    ...
    for inputs, targets in tqdm(train_loader, desc=f"Epoch [{epoch+1}/{num_epochs}]", unit="batch"):
        ...
        with autocast(device_type=device_type):
            ...
            # 多输出融合损失
            losses = [criterion_mse(output, targets) * (0.5 ** i) for i, output in enumerate(outputs)]
